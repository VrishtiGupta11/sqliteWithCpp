...
void linearSearch(string toSearch) {
	int i = 0;
	while (i < 6) {
		if (rollNo[i] == toSearch) {
			cout << "Roll no. " << toSearch << " found at position " << i + 1 << endl;
			break;
		}
		i++;
		if (i == 6) {
			cout << "Roll no. " << toSearch << " not found!\n";
		}
	}
}
